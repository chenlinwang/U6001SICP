...
(((() 1) 2) 3)
#+END_SRC
The =op= should be associative, which means:
...
** Answer
For each time the board needs to add a new number, the programs starts a recursion on =queen-col=.So it grows in exponenial, as for n, it takes about \Theta((n!)^{2}).
