...
(define travel-distance-simple
  (lambda (elevation velocity angle)
    (* (time-to-impact (* (sin (degree2radian angle)) velocity) elevation) (* (cos (degree2radian angle)) velocity))))
...
(define (find-best-angle velocity elevation)
  (define speed 10.0)
  (define (delta-intervel increment)
    (* speed increment 1.1))
  (define (speed-up-calculation start end current-increment)
    (define current-best-angle
      (find-best-angle-by-step velocity elevation start end current-increment 0 -1))
;    (display current-increment)
;    (newline)
    (if (< current-increment alpha-increment)
        current-best-angle
        (speed-up-calculation
         (- current-best-angle (delta-intervel current-increment))
         (+ current-best-angle (delta-intervel current-increment))
         (/ current-increment speed))))
  (speed-up-calculation 0 90 1))

(define (find-best-angle-by-step velocity elevation start end increment best-angle best-length)
  (define present-distance (travel-distance-simple elevation velocity start))
;  (newline)
;  (display start)
  (if (> start end)
      best-angle
      (if (> present-distance best-length)
          (find-best-angle-by-step velocity elevation (+ start increment) end increment (+ start increment) present-distance)
          (find-best-angle-by-step velocity elevation (+ start increment) end increment best-angle best-length))))
...
(define (integrate x0 y0 u0 v0 dt g m beta)
    (define (muldt x v)
      (+ x (* v dt)))
    (define (sqrtspeed v)
      (* (/ 1 m) (sqrt (+ (square u0) (square v0))) beta v))
    (if (< y0 0)
        x0
        (integrate (muldt x0 u0) (muldt y0 v0) (muldt u0 (- (sqrtspeed u0))) (muldt v0 (- 0 (sqrtspeed v0) g)) dt g m beta)))

(define (travel-distance elevation velocity angle)
  (integrate 0 elevation (* velocity (cos (degree2radian angle))) (* velocity (sin (degree2radian angle))) 0.01 9.8 mass beta))


;Test for different angle for the same speed.

(define test-angle-increment 1)
(define test-elevation 1.2)
(define test-speed 45)

(define (test-for-angle start end)
  (newline)
  (display (travel-distance test-elevation test-speed start))
  (if (> start end)
      (newline)
      (test-for-angle (+ start test-angle-increment) end)))
...
(define (integrate-y-time y0 u0 v0 dt g m beta ttotal)
    (define (muldt x v)
      (+ x (* v dt)))
    (define (sqrtspeed v)
      (* (/ 1 m) (sqrt (+ (square u0) (square v0))) beta v))
    (if (< y0 0)
        ttotal
        (integrate-y-time (muldt y0 v0) (muldt u0 (- (sqrtspeed u0))) (muldt v0 (- 0 (sqrtspeed v0) g)) dt g m beta (+ ttotal dt))))

(define (calculate-time velocity angle)
  (integrate-y-time 0 (* velocity (cos (degree2radian angle))) (* velocity (sin (degree2radian angle))) 0.01 9.8 mass beta 0))

(define (p6 velocity distance delta)
  ;single test
  (define (single-test angle)
    (define present-distance (travel-distance 0 velocity angle))
    (if (and (< distance present-distance) (> (+ distance delta) present-distance))
        present-distance
        0))

  ;main test
  (define (p6-test-angle start end dangle)
    (define present-distance (single-test start))
    (if (< start end)
        (if (= present-distance 0)
            (p6-test-angle (+ start dangle) end dangle)
            (begin (newline)
                   (display start)
                   (display "\t->\t")
                   (display (calculate-time velocity start))
                   (display "\t->\t")
                   (display present-distance)
                   (p6-test-angle (+ start dangle) end dangle)))
        (newline)))
  (p6-test-angle 0 90 0.1))
