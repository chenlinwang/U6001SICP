  ...
  (let ((chamber (create-place 'chamber-of-stata)))
    (create-spell
     ...
     (lambda (caster target)
       (if (ask target 'is-a 'person)
           ...
           (ask caster 'say (list 'can
                                  'not
                                  'cast
                                  'spell
                                  'on
                                  (ask target 'name))))))
    (create-counter-spell
     'counter-boil-spell
     'boil-spell
     chamber
     "valilio"
     (lambda (caster target)
       (ask caster 'emit
            '(a transparent block appeared and the heat is blocked))))
    ...
    (create-counter-spell
     ...
     (lambda (caster target)
       (lambda (caster target)
       (ask caster 'emit
            '(a transparent block appeared and the spell is blocked)))))
    (create-spell
     'wind-of-doom
     chamber
     "windiano doomino"
    ...
    (create-counter-spell
     ...
     (lambda (caster target)
       (lambda (caster target)
       (ask caster 'emit
            '(a transparent block appeared and the wind is blocked)))))
    (create-spell
     'transport
     chamber
     "movido tadobio"
     (lambda (caster target)
       (cond ((ask target 'is-a 'mobile-thing)
              (let ((new-loc (pick-random all-rooms)))
                (ask target 'emit (list 'suddernly
                                        (ask target 'name)
                                        'disappeared
                                        'at
                                        'the
                                        ...
                                        'at
                                        (ask new-loc 'name)))
                (ask target 'change-location new-loc)))
             (else
              (ask caster 'say (list (ask target 'name)
                                     'can
                                     'not
                                     'be
                                     'moved))))))
    (create-counter-spell
     'counter-transport-spell
     'transport
     chamber
     "valilio"
     (lambda (caster target)
...
;uncomment after writing professors
    ...
    ;; give each students a wand
    (map (lambda (s) (create-wand '12-inch-wooden s)) students)
    (append students
